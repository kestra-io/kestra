...
public final class ExecutableUtils {
    ...
    public static TaskRun manageIterations(TaskRun taskRun, Execution execution, boolean transmitFailed, boolean allowFailure) throws InternalException {
        ...
        if (previousTaskRun.getOutputs() == null) {
            if (currentState == State.Type.SUCCESS) {
                // FIXME there is a bug when we transition from RUNNING to SUCCESS we loose the previous output
                // the best we can do is to consider that everything has been created and transitioned to RUNNING
                // testing shows that this is right 90% of the time so there is still a slight risk of the iteration counters being false (usually negative running at the end of the execution)
                log.warn("Unable to find previous taskrun outputs, iteration counters may not be accurate");
                iterations = new HashMap<>();
                iterations.put(State.Type.CREATED.toString(), 0);
                iterations.put(State.Type.RUNNING.toString(), numberOfBatches);
            } else {
                iterations = new HashMap<>();
            }
        } else {
            iterations = (Map<String, Integer>) previousTaskRun.getOutputs().get("iterations");
        }

        int currentStateIteration = getIterationCounter(iterations, currentState, numberOfBatches) + 1;
        iterations.put(currentState.toString(), currentStateIteration);
        if (previousState.isPresent() && previousState.get() != currentState) {
            int previousStateIterations = getIterationCounter(iterations, previousState.get(), numberOfBatches) - 1;
