...
// Loaded on demand: vueFlowUtils statically pulls @vue-flow/core, and this store is
// imported eagerly, so a static import would put the graph renderer on every page.
const loadVueFlowUtils = () => import("@kestra-io/topology/vue-flow-utils")
...
export const usePlaygroundStore = defineStore("playground", () => {
    ...
    async function replayOrTriggerExecution(taskId?: string, breakpoints?: string[], graph?: any) {
        ...
        if (lastExecution && taskId && graph
            ...
            const {areTasksIdenticalInGraphUntilTask} = await loadVueFlowUtils()
            if (areTasksIdenticalInGraphUntilTask(lastExecution.graph, graph, taskId)) {
    ...
    async function getNextTaskIds(taskId?: string) {
        ...
        const {getNextTaskNodes} = await loadVueFlowUtils()
        const nextTasksNodes = getNextTaskNodes(graph, taskNode)
