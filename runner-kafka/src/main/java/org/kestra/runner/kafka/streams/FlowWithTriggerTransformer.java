...
@Slf4j
public class FlowWithTriggerTransformer implements Transformer<String, KafkaExecutor.ExecutionWithFlow, Iterable<KeyValue<String, FlowTriggerWithExecution>>> {
    ...
    @Override
    public Iterable<KeyValue<String, FlowTriggerWithExecution>> transform(String key, KafkaExecutor.ExecutionWithFlow value) {
        try (KeyValueIterator<String, ValueAndTimestamp<Flow>> flows = this.flowStore.all()) {
            ...
            return flowService.flowWithFlowTrigger(allFlows.stream())
                .stream()
                .map(f -> KeyValue.pair(f.getFlow().uidWithoutRevision(), new FlowTriggerWithExecution(f.getFlow(), value.getExecution())))
                .collect(Collectors.toList());
        }
    }
