...
// - Otherwise build the SDK package from its committed source. build:sdk builds the shared
//   @kestra-io/hey-api-plugin FIRST (the SDK bundles its runtime), then the SDK itself.
//
// To refresh the SDK after the OSS API changes, run `npm run generate:sdk` explicitly (that is the
// only path that invokes Gradle)
...
try {
    execSync("npm run build:sdk", {stdio: "inherit", cwd: uiRoot})
} catch {
