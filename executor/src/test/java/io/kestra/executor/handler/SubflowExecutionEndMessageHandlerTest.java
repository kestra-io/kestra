...
class SubflowExecutionEndMessageHandlerTest {
    ...
    void shouldEmitSubflowExecutionResultWhenChildExecutionEnds() {
        ...
        assertThat(harness.subflowExecutionResultQueue().emitted()).as("the result created from the child execution is emitted on the subflow execution result queue").hasSize(1);
    ...
    void shouldNotEmitSubflowExecutionResultWhenTaskDoesNotWaitForExecution() {
        ...
        harness.subflowExecutionEndMessageHandler().handle(message);
        assertThat(harness).emittedNoSubflowResults();
    }
    ...
    void shouldNotEmitSubflowExecutionResultWhenChildExecutionKillSwitched() {
        ...
        assertThat(harness).as("nothing is emitted and the parent kill switch is never even evaluated").emittedNoSubflowResults();
