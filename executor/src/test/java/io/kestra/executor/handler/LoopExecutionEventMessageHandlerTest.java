...
class LoopExecutionEventMessageHandlerTest {
    ...
    void shouldTerminateLoopWithSuccessOnLastIteration() throws InternalException {
        ...
        assertThat(harness).emittedNoExecutions();
    }
    ...
    void shouldEmitNextIterationWhenMoreIterationsRemain() throws InternalException {
        ...
        assertThat(maybeExecutor).as("handler emits the next loop iteration on the execution queue and returns empty (null from inner lambda)").isEmpty();
    ...
    void shouldAccumulateTerminatedIterationsPerStateWhenTransmitFailedIsDisabled() throws InternalException {
        ...
        assertThat(maybeExecutor).as("the loop keeps running (emits the last iteration) and records both terminal states").isEmpty();
        assertThat(taskOutputService.getOutputs(loopTaskRun))
    ...
    @Test
    void shouldFailOnlyThisExecutionWhenLoopValuesShrinkBetweenIterations() throws InternalException {
        // Given — the saved iterationCount (3) no longer matches the re-resolved values (1
        // element): the source shrank between iterations (in production, e.g. a kv() expression
        // re-resolved when the next iteration is picked)
        var flow = Flows.of(loopFlowWithValues(List.of("a")));
        harness.registerFlow(flow);
        var execution = Execution.newExecution(flow, Collections.emptyList());
        String loopTaskRunId = IdUtils.create();
        var loopTaskRun = loopTaskRun(loopTaskRunId, execution);
        ...
        assertThat(maybeExecutor).as("fails only this execution, does not throw and crash the whole instance").isPresent();
        var taskRun = maybeExecutor.get().getExecution().findTaskRunByTaskRunId(loopTaskRunId);
        assertThat(taskRun.getState().getCurrent()).isEqualTo(State.Type.FAILED);
    }
    ...
    private Flow loopFlowWithValues(List<String> values) {
