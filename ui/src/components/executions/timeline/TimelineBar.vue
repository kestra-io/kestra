<template>
    <template v-if="execution">
        <KsPopover
            ...
            :popperOptions="POPOVER_POPPER_OPTIONS"
            :width="280"
            :showArrow="true"
        >
            <template #reference>
...
<script setup lang="ts">
    ...
    watch(popoverVisible, (visible) => {
        ...
        if (!barRef.value || !(card instanceof HTMLElement)) return
        const spaceAbove = barRef.value.getBoundingClientRect().top - card.getBoundingClientRect().top
        popoverPlacement.value = spaceAbove < POPOVER_MIN_SPACE_ABOVE_PX ? "bottom" : "top"
    })
