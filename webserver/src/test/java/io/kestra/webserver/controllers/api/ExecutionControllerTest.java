...
class ExecutionControllerTest {
   ...
   void shouldHaveAnUrlWhenCreated() {
    ...
    void shouldRemoveLabelsFromExecution() throws QueueException, TimeoutException {
        ...
        String flowId = "restart_with_inputs";
        // Create an execution with labels
        Label label1 = new Label("key1", "value1");
        Label label2 = new Label("key2", "value2");

        Execution execution = runnerUtils.runOne(null, TESTS_FLOW_NS, flowId, null, null);

        // First add some labels to the execution
        Execution executionWithLabels = client.toBlocking().retrieve(
            HttpRequest
                .POST("/api/v1/executions/" + execution.getId() + "/labels",
        ...
        assertThat(executionWithLabels.getLabels().getFirst(), is(new Label("key1", "value1")));
        assertThat(executionWithLabels.getLabels().get(1), is(new Label("key2", "value2")));
