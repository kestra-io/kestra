...
public abstract class AbstractScheduler implements Scheduler {
    ...
    protected final void handle() {
        ...
        this.handleNext(this.flowListeners.flows(), now, (triggers, scheduleContext) -> {
            processTriggers(triggers, scheduleContext, now);
        });

        counter.decrementAndGet();
    }

    protected void processTriggers(List<Trigger> triggers, ScheduleContextInterface scheduleContext, ZonedDateTime now) {
        if (triggers.isEmpty()) {
            return;
        }
