...
class KillingDecisionTest {
    ...
    void shouldKillCreatedTaskRunWhenExecutionIsKilling() {
        ...
        assertThat(context).as(
            "handleCreatedKilling turns the CREATED taskrun into a KILLED WorkerTaskResult and merges it through addWorkerTaskResults — the from-label recorded for the merge is 'addWorkerTaskResult' (handleCreatedKilling never calls withExecution under its own name). With the taskrun now terminal, handleKilling closes the execution in the same cycle."
        )
    ...
    void shouldKillExecutionWhenAllTaskRunsAlreadyTerminal() {
        ...
        assertThat(context).as("execution KILLED via handleKilling; t2 never got a taskrun")
            .executionInState(State.Type.KILLED)
            .updatedFrom("handleKilling")
            .transitioned(State.Type.KILLING, State.Type.KILLED)
            .hasNoWorkerTasks()
    ...
    void shouldStayKillingWhenATaskRunIsStillRunning() {
        ...
        assertThat(context).as("the executor waits for the worker — no KILLED transition, no new work, nothing changed")
    ...
    void shouldKillFlowableParentWithTerminalAttemptWhenStartedChildrenAreTerminal() {
        ...
        Assertions.assertThat(cycle1.getException()).as(
            "the KILLING wait-branch of childWorkerTaskResult resolves the parent to KILLED via withStateAndAttempt — the last attempt must be terminal too (the attempt-termination fix), not just the taskrun state"
        ).isNull();
        assertThat(cycle1)
        ...
        assertThat(cycle2).as("with the only top-level task (seq) now terminal, handleEnd/onEnd computes the final state — guessFinalState yields KILLED because a taskrun is KILLED")
            .executionInState(State.Type.KILLED)
            .updatedFrom("onEnd");
    }
    ...
    void shouldKillExecutionWhenWorkerReportsKilledForRunningTaskRun() throws Exception {
        ...
        assertThat(context).as("the KILLED taskrun is merged and, all taskruns now terminal, handleKilling closes the execution")
            .hasTaskRunInState("t1", State.Type.KILLED)
            .executionInState(State.Type.KILLED)
            .updatedFrom("handleKilling")
            .transitioned(State.Type.KILLING, State.Type.KILLED)
