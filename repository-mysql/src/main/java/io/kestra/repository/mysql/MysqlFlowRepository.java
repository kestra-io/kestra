...
public class MysqlFlowRepository extends AbstractFlowRepository {
    ...
    public ArrayListTotal<SearchResult<Flow>> findSourceCode(String query, Pageable pageable) {
        ...
        if (query != null) {
            select.and(DSL.condition("MATCH (source_code) AGAINST (? IN NATURAL LANGUAGE MODE)", query));
        }

        Select<Record> limit = this.jdbcRepository.pageable(select, pageable);

        return this.jdbcRepository.fetchPage(
            limit,
            record -> new SearchResult<>(this.jdbcRepository.map(record), this.jdbcRepository.fragments(query, record.getValue("source_code", String.class)))
        );
    }
}
