...
class ExecutionEventMessageHandlerTest {
    ...
    void shouldStampTheClaimedConcurrencyScopesOnAdmission() {
        ...
        assertThat(maybeExecutor).as("the execution remembers the scopes it claimed a slot in, so the release decrements exactly these even if the limit definition changes while it runs")
            .isPresent();
    ...
    void shouldCreateSlaMonitorForNonLoopExecution() {
        ...
        var flow = Flows.of(Fixtures.flowWithSla(sla));
        harness.registerFlow(flow);
        var execution = Execution.newExecution(flow, null, Collections.emptyList(), Optional.empty(), ExecutionKind.NORMAL);
        harness.executionStateStore().save(execution);

        // When
        executionEventMessageHandler.handle(new ExecutionEvent(execution, ExecutionEventType.CREATED));
        assertThat(harness.slaMonitorStateStore().pending()).hasSize(1);
    }
