...
/**
 * Base class for processors executing {@link WorkerTask} jobs.
 * <p>
 * Holds the single-task run pipeline shared by all task processors: kill-check, task-output
 * caching, attempt execution, terminal-state adjustment, result emission, and metrics.
 */
public abstract class AbstractWorkerTaskProcessor extends AbstractWorkerJobProcessor<WorkerTask> {

    protected final String workerId;

    // SERVICES
    protected final RunContextInitializer runContextInitializer;
    protected final RunContextLoggerFactory runContextLoggerFactory;
    ...
    // QUEUEs
    protected final WorkerQueue<WorkerTaskResult> workerTaskResultQueue;
    private final WorkerQueue<MetricEntry> workerMetricQueue;

    protected AbstractWorkerTaskProcessor(final String workerId,
    ...
    protected WorkerTaskResult runTask(WorkerTask workerTask, boolean cleanUp) {
        return runTask(workerTask, cleanUp, null);
    }

    protected WorkerTaskResult runTask(WorkerTask workerTask, boolean cleanUp, DefaultRunContext providedRunContext) {
        ...
        try {
            ...
            if (isTaskCacheEnabled(workerTask)) {
                ...
                if (hash.isPresent()) {
                    Optional<WorkerTaskResult> cached = getCachedResult(runContext, workerTask, hash.get());
                    if (cached.isPresent()) {
                        return cached.get();
                    }
                }
            ...
            if (isTaskCacheEnabled(workerTask) && hash.isPresent() && (state == State.Type.SUCCESS || state == State.Type.WARNING)) {
                uploadCacheEntry(runContext, workerTask, taskRunWithOutput, taskRun, hash.get());
            }
    ...
    protected void logTerminated(WorkerTask workerTask, TaskRun taskRun) {
    ...
    private boolean isTaskCacheEnabled(WorkerTask workerTask) {
        return workerTask.getTask().getTaskCache() != null && workerTask.getTask().getTaskCache().getEnabled();
    }

    private Optional<WorkerTaskResult> getCachedResult(RunContext runContext, WorkerTask workerTask, String hash) {
        try {
            Optional<InputStream> cacheFile = runContext.storage().getCacheFile(hash, workerTask.getTaskRun().getValue(), workerTask.getTask().getTaskCache().getTtl());
            if (cacheFile.isPresent()) {
                ...
                try (ZipInputStream archive = new ZipInputStream(cacheFile.get())) {
                    if (archive.getNextEntry() != null) {
                        ...
                        workerTaskResultQueue.put(workerTaskResult);
                        return Optional.of(workerTaskResult);
                    }
                }
            }
    ...
    private void uploadCacheEntry(RunContext runContext, WorkerTask workerTask, TaskRunWithOutput taskRunWithOutput, TaskRun taskRun, String hash) {
        ...
        try (
            ...
            URI uri = runContext.storage().putCacheFile(archiveFile.toFile(), hash, taskRun.getValue());
