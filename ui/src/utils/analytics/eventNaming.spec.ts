...
describe("resolvePosthogEventName", () => {
    ...
    ])("resolves %s with %j to %s", (type, properties, expected) => {
    ...
    it("matches the surface.object.action shape for every mapped output", () => {
        const mappedNames = [
            "app.flow.executed",
            "app.ai-copilot.invoked",
            "app.blueprint.used",
            "app.survey.submitted",
            "app.survey.skipped",
            "app.account.created",
            "app.account.creation-failed",
            "app.marketing-survey.submitted",
            "app.marketing-survey.skipped",
            "app.setup-flow.completed",
            "app.error.occurred",
            "app.editor-tab.opened",
            "app.editor-tab.closed",
            "app.plugin-doc.viewed",
            "app.editor-files.opened",
            "app.editor-blueprint.selected",
            "app.forgot-password.clicked",
            "app.oss-auth.completed",
            "app.onboarding-step.viewed",
            "app.onboarding-step.advanced",
            "app.onboarding-step.auto-advanced",
            "app.onboarding-step.validation-failed",
            "app.onboarding.completed",
            "app.onboarding.cancelled",
        ]

        mappedNames.forEach((name) => {
            expect(name).toMatch(/^app\.[a-z0-9-]+\.[a-z0-9-]+$/)
        })
    })
})
