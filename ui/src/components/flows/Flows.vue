            ...
            >
                <template #navbar>
                    <KSFilter
                        ...
                        :defaultScope="
                            !route.name?.toString().startsWith('namespaces/')
                        "
                    />
                </template>
                ...
                <template #table>
                    ...
                    >
                        ...
                        <template #default>
                            ...
                            <template
                                v-for="colProp in displayColumns"
                                :key="colProp"
                            >
                                <el-table-column
                                    v-if="colProp === 'labels'"
...
<script setup lang="ts">
    ...
    import {
        DataTableRef,
        useDataTableActions,
    } from "../../composables/useDataTableActions";
    ...
    const {visibleColumns: displayColumns, updateVisibleColumns} =
        useTableColumns({
    ...
    const canCheck = computed(
        () => canRead.value || canDelete.value || canUpdate.value,
    );
    const canCreate = computed(() =>
        user.value?.hasAnyActionOnAnyNamespace(permission.FLOW, action.CREATE),
    );
    const canRead = computed(() =>
        user.value?.isAllowed(permission.FLOW, action.READ, route.query.namespace),
    );
    ...
    function loadData(callback?: () => void) {
        ...
        flowStore
            ...
            .then((data: any) => {
                if (
                    user.value?.hasAnyActionOnAnyNamespace(
                    ...
                    executionsStore
                        .loadLatestExecutions({
                            flowFilters: data.results.map((flow: any) => ({
                                id: flow.id,
    ...
    const {queryWithFilter, onPageChanged, onRowDoubleClick, onSort, ready} =
        useDataTableActions({
    ...
    const selectionIds = computed(() =>
        selection.value.map((flow) => ({id: flow.id, namespace: flow.namespace})),
    );
    ...
    function exportFlows() {
        toast.confirm(
            t("flow export", {
                flowCount: queryBulkAction.value
                    ? flowStore.total
                    : selection.value.length,
            }),
            () => {
                const flowCount = queryBulkAction.value
                    ? flowStore.total
                    : selection.value.length;
                if (queryBulkAction.value) {
                    ...
                    return flowStore
                        .exportFlowByIds({ids: selection.value})
                        .then(() => {
                            toast.success(
                                t("flows exported", {count: flowCount}),
                            );
                            toggleAllUnselected();
                        });
                }
    ...
    function disableFlows() {
        toast.confirm(
            ...
            () => {
                if (queryBulkAction.value) {
                    return flowStore
                        .disableFlowByQuery(loadQuery())
                        .then((r: any) => {
                            toast.success(
                                t("flows disabled", {count: r.data.count}),
                            );
                            toggleAllUnselected();
                            loadData(() => {});
                        });
                    ...
                    return flowStore
                        .disableFlowByIds({ids: selectionIds.value})
                        .then((r: any) => {
                            toast.success(
                                t("flows disabled", {count: r.data.count}),
                            );
    ...
    function enableFlows() {
        toast.confirm(
            ...
            () => {
                if (queryBulkAction.value) {
                    return flowStore
                        .enableFlowByQuery(loadQuery())
                        .then((r: any) => {
                            toast.success(
                                t("flows enabled", {count: r.data.count}),
                            );
                            toggleAllUnselected();
                            loadData(() => {});
                        });
                    ...
                    return flowStore
                        .enableFlowByIds({ids: selectionIds.value})
                        .then((r: any) => {
                            toast.success(
                                t("flows enabled", {count: r.data.count}),
                            );
    ...
    function deleteFlows() {
        toast.confirm(
            ...
            () => {
                if (queryBulkAction.value) {
                    return flowStore
                        .deleteFlowByQuery(loadQuery())
                        .then((r: any) => {
                            toast.success(
                                t("flows deleted", {count: r.data.count}),
                            );
                            toggleAllUnselected();
                            loadData(() => {});
                        });
                    ...
                    return flowStore
                        .deleteFlowByIds({ids: selectionIds.value})
                        .then((r: any) => {
                            toast.success(
                                t("flows deleted", {count: r.data.count}),
                            );
        ...
        if (file.value && file.value.files && file.value.files[0]) {
            ...
            flowStore.importFlows(formData as any).then((res: any) => {
                if (res.data.length > 0) {
                    toast.warning(
                        t("flows not imported") + ": " + res.data.join(", "),
                    );
                } else {
    ...
    function loadQuery(base?: any) {
        ...
        if (props.namespace) {
            queryFilter["filters[namespace][PREFIX]"] =
                route.params.id || props.namespace;
        }
    ...
    async function openExecuteModal(row: any) {
        ...
        executionsStore.flow = row;
        showRunModal.value = true;
    }

    async function exportFlowsAsStream() {
        await flowStore.exportFlowAsCSV(route.query);
    }
</script>
