...
import io.kestra.executor.testkit.EmissionJournal;
...
class ClosedLoopTest {
    ...
    void oneDeliveryIsOneProductionStep() {
        ...
        List<EmissionJournal.Entry> emitted = harness.step(created);
        ...
        assertThat(emitted).extracting(EmissionJournal.Entry::queue).containsExactly("workerJobEvent", "executionEvent", "followExecutionEvent");
    ...
    void sequentialTasksRunToSuccessThroughTheLoop() {
        ...
        assertThat(terminal.getFirst().emitted())
            .extracting(EmissionJournal.Entry::queue)
    ...
    void terminationReleasesTheSlotAndRequeuesBeforeAnnouncingTheEnd() {
        ...
        EmissionJournal.Entry requeued = releasing.emitted("execution").findFirst().orElseThrow();
        EmissionJournal.Entry terminated = releasing.emitted("executionEvent").findFirst().orElseThrow();
    ...
    void scheduledExecutionWaitsForTheDelayLoopAndTheClock() {
        ...
        List<EmissionJournal.Entry> onArrival = harness.step(scheduled);

        assertThat(onArrival).as("nothing goes to a worker; a delay is parked").extracting(EmissionJournal.Entry::queue).doesNotContain("workerJobEvent");
        ...
        List<EmissionJournal.Entry> onResume = harness.tickExecutionDelays(T0.plus(Duration.ofHours(2)));
        assertThat(onResume).extracting(EmissionJournal.Entry::queue).contains("executionEvent");
