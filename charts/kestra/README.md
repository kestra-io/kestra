...
| common.progressDeadlineSeconds | string | `""` | Seconds a Deployment rollout may make no progress before the controller reports `ProgressDeadlineExceeded`. Unset uses the Kubernetes default of 600, which a large image can exceed on a cold pull. Deployments only. |
