...
export const useDashboardStore = defineStore("dashboard", () => {
    const dashboardList =
        ref<{ id: string; title: string; isDefault: boolean }[]>();
    ...
    const parsedSource = computed<
        { id?: string; [key: string]: any } | undefined
    >((previous) => {
        try {
    ...
    async function list(
        ...
        const response = await axios.get(
            `${apiUrl()}/dashboards?size=100${sort ? `&sort=${sort}` : ""}`,
            {params},
        );
        ...
        dashboardList.value = res.results.map((dashboard) => {
            const isADefaultForThisRoute = isAdminDefinedDefaultDashboard(
                dashboard.id,
                route,
            );
        ...
        if (!isThereADefault) {
            const defaultDashboardBundledInUI = {
                id: "default",
                title: t("dashboards.default"),
                isDefault: true,
            };
            dashboardList.value = [
                defaultDashboardBundledInUI,
                ...dashboardList.value,
            ];
        }
    ...
    async function saveDefaults(defaultDashboardsRequest: {
        ...
        const def = {
            ...loadedDef,
            ...defaultDashboardsRequest,
            defaultHomeDashboard: normalizeDefault(
                defaultDashboardsRequest.defaultHomeDashboard,
            ),
            defaultFlowOverviewDashboard: normalizeDefault(
                defaultDashboardsRequest.defaultFlowOverviewDashboard,
            ),
            defaultNamespaceOverviewDashboard: normalizeDefault(
                defaultDashboardsRequest.defaultNamespaceOverviewDashboard,
            ),
        };

        const res = await axios.post(
            `${apiUrlWithoutTenants()}/tenants/main/settings/default-dashboards`,
            def,
    ...
    const getDashboardId = async (route: RouteLocation): Promise<string> => {
        ...
        if (!routeName || !DASHBOARD_ROUTES.includes(routeName)) {
            throw new Error(
                "invalid route in getDashboard: " + routeName?.toString(),
            );
        }
        ...
        if (
            ...
            route.params.dashboard !== "default"
        ) {
    ...
    function getUserDashboardStorageKey(route: RouteLocation) {
        ...
        if (!tenant) {
            throw new Error(
                "tenant is mandatory in getUserDashboardStorageKey",
            );
        }
        ...
        switch (dashboardType) {
            case "DASHBOARD_MAIN":
                return Promise.resolve(
                    defaultDashboards.value?.defaultHomeDashboard,
                );
            case "DASHBOARD_NAMESPACE":
                return Promise.resolve(
                    defaultDashboards.value?.defaultNamespaceOverviewDashboard,
                );
            case "DASHBOARD_FLOW":
                return Promise.resolve(
                    defaultDashboards.value?.defaultFlowOverviewDashboard,
                );
        }
    }

    const isAdminDefinedDefaultDashboard = (
        dashboardId: string,
        route: RouteLocation,
    ): boolean => {
        const dashboardType = getDashboardType(route);
                ...
                case "DASHBOARD_NAMESPACE":
                    return (
                        defaultDashboards.value
                            ?.defaultNamespaceOverviewDashboard === dashboardId
                    );
                case "DASHBOARD_FLOW":
                    return (
                        defaultDashboards.value
                            ?.defaultFlowOverviewDashboard === dashboardId
                    );
            }
        }
        return false;
    ...
    const getDefaultDashboard = async (
        ...
        switch (dashboardType) {
            case "DASHBOARD_MAIN":
                return defaultDashboards.value?.defaultHomeDashboard;
            case "DASHBOARD_NAMESPACE":
                return defaultDashboards.value
                    ?.defaultNamespaceOverviewDashboard;
            case "DASHBOARD_FLOW":
                return defaultDashboards.value?.defaultFlowOverviewDashboard;
        }
    };
    ...
    async function load(id: Dashboard["id"]): Promise<Dashboard | undefined> {
        let response;
        try {
            response = await axios.get(`${apiUrl()}/dashboards/${id}`, {
                validateStatus,
            });
        } catch {
            return undefined;
    ...
    async function generate(
        ...
        const response = await axios.post(
            `${apiUrl()}/dashboards/${id}/charts/${chartId}`,
            parameters,
            {validateStatus},
        );
        return response.data;
    ...
    async function validateChart(source: string) {
        const response = await axios.post(
            `${apiUrl()}/dashboards/validate/chart`,
            source,
            header,
        );
    ...
    async function chartPreview(request: Request) {
        const response = await axios.post(
            `${apiUrl()}/dashboards/charts/preview`,
            request,
        );
        return response.data;
    }
    ...
    async function exportDashboard(
        ...
        const path = isDefault
            ...
            : `/${dashboard.id}/charts/${chart.id}/export/to-csv`;
        const payload = isDefault
            ? {chart: chart.content, globalFilter: parameters}
            : parameters;
    ...
    function recursivelyLoopUpSchemaRef(
        a: any,
    ...
    async function loadChart(chart: any) {
        ...
        if (selectedChart.value?.content === yamlChart) {
            return {
                error:
                    chartErrors.value.length > 0 ? chartErrors.value[0] : null,
                data: selectedChart.value
                    ? {...selectedChart.value, raw: chart}
                    : null,
                raw: chart,
            };
        }
        ...
        _throttle(
            async () => {
                ...
                if (
                    !isCreating.value &&
                    dbId !== undefined &&
                    YAML_UTILS.parse(sourceCode.value).id !== dbId
                ) {
                    coreStore.message = {
                        variant: "error",
