...
describe("useVersionUpgradeNotice", () => {
    ...
    it("keeps one dismissal per instance", () => {
        setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}}).dismiss()

        setActivePinia(createPinia())
        setup({configs: {uuid: "i2", versionUpgrade: UPGRADE}}).dismiss()

        // Dismissing on i2 must not bring the banner back on i1.
        setActivePinia(createPinia())
        expect(setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}}).visible.value).toBe(false)
    })

    it("honours a dismissal persisted in the single-instance shape", () => {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({instanceUuid: "i1", dismissedVersion: UPGRADE.to}))

        expect(setup({configs: {uuid: "i1", versionUpgrade: UPGRADE}}).visible.value).toBe(false)
    })

    it("hides the notice for the session when the instance reports no uuid", () => {
        const {visible, dismiss} = setup({configs: {versionUpgrade: UPGRADE}})
