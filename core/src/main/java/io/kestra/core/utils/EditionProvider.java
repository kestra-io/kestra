...
public class EditionProvider {
    ...
    /**
     * Persists the current edition in the settings repository. Must be called after
     * database migrations have completed (e.g., from a startup hook).
     *
    ...
    public void persistEdition(SettingRepositoryInterface settingRepository) {
        ...
        if (editionSetting.isEmpty() || !editionSetting.get().getValue().equals(edition)) {
            settingRepository.save(Setting.builder().key(Setting.INSTANCE_EDITION).value(edition).build());
        }
    }
