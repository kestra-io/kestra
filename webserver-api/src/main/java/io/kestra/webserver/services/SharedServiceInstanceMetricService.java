...
public class SharedServiceInstanceMetricService {
    ...
    void populateSharedServiceInstanceMetrics() {
        ...
        var sumedServiceInstanceMetrics = serviceInstances.stream()
            ...
            .flatMap(
                serviceInstance -> serviceInstance.metrics()
                    .stream()
                    .filter(
                        metric -> metricConfig.getSharedServiceInstanceMetrics().get(serviceInstance.type()).contains(metric.name())
                    )
            ).collect(
        ...
        sumedServiceInstanceMetrics.forEach((metricKey, value) ->
        {
            ...
            sharedMetricsGauges.computeIfAbsent(
                metricKey, (mk) -> metricRegistry.gauge(
