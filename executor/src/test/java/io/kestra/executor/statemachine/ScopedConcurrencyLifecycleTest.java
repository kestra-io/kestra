...
class ScopedConcurrencyLifecycleTest {
    ...
    void shouldResolveOnlyTheFlowScopeInOss() {
        ...
        FlowWithSource unlimited = Flows.of(Flows.log());
    ...
    void shouldApplyTheFirstReachedLimitAcrossNamespaces() {
        ...
        harness.start(flowA);
        assertThat(harness).hasRunning(ioKestraScope, 10);

        // When: running another flow A
        Execution another = Executions.created(flowA);
        ExecutorContext context = harness.handle(another);

        assertThat(context)
            .as("the io.kestra namespace limit is the first one reached, so the execution FAILs — the flow (1 < 5) and io.kestra.example1 (1 < 100) limits pass and claim nothing")
            .executionInState(State.Type.FAILED).updatedFrom("handleConcurrencyLimit");
    ...
    void shouldApplyTheFlowBehaviorWhenFlowAndNamespaceLimitsAreBothReached() {
        ...
        assertThat(context).as("the flow limit is evaluated first, so its QUEUE behavior wins over FAIL").executionInState(State.Type.QUEUED).updatedFrom("handleConcurrencyLimit");
    ...
    void shouldApplyTheInnermostNamespaceBehaviorBeforeItsParent() {
        ...
        assertThat(context).as("innermost first — CANCELLED, not FAILED").executionInState(State.Type.CANCELLED).updatedFrom("handleConcurrencyLimit");
    }
    ...
    void shouldApplyTheTenantLimitWhenEveryNamespaceHasCapacity() {
        ...
        assertThat(context).as("the tenant limit, last in the evaluation order, defines the behavior").executionInState(State.Type.FAILED).updatedFrom("handleConcurrencyLimit");
        assertThat(harness).hasRunning(tenantScope, 1);
    }
    ...
    void shouldClaimOneSlotInEveryScopeWhenRunning() {
        ...
        assertThat(started).as("one slot claimed per scope").executionInState(State.Type.RUNNING);
        assertThat(harness).hasRunning(flow, 1);
        assertThat(harness).hasRunning(childScope, 1);
        assertThat(harness).hasRunning(parentScope, 1);
        assertThat(harness).hasRunning(tenantScope, 1);
    }
    ...
    void shouldCountExecutionsOfFlowsWithoutOwnLimitTowardTheNamespaceLimit() {
        ...
        assertThat(context).as("the third queues on the namespace limit; no flow-scoped counter ever moved").executionInState(State.Type.QUEUED).updatedFrom("handleConcurrencyLimit");
        assertThat(harness).hasRunning(namespaceScope, 2);
    ...
    void shouldReleaseEveryScopeOnTermination() {
        ...
        Assertions.assertThat(popped).as("every scope counter is back to zero, nothing to pop").as("nothing popped").isEmpty();
        assertThat(harness).hasRunning(flow, 0);
        assertThat(harness).hasRunning(namespaceScope, 0);
        assertThat(harness).hasRunning(tenantScope, 0);
    }
    ...
    void shouldPopQueuedExecutionOfAnotherFlowWhenTheSharedNamespaceSlotFrees() {
        ...
        ExecutorContext runningA = harness.start(flowA);
        Execution waitingB = Executions.created(flowB);
        harness.executionStateStore().save(waitingB);
        assertThat(harness.handle(waitingB)).executionInState(State.Type.QUEUED);

        // When: A terminates
        Optional<Execution> popped = release(terminated(flowA, runningA.getExecution()));

        Assertions.assertThat(popped).as("B — an execution of a different flow — takes over the namespace slot")
        ...
        assertThat(harness).hasRunning(namespaceScope, 1);
        assertThat(harness).hasNothingQueued();
    }
    ...
    void shouldSkipCandidateBlockedByItsOwnFlowLimitAndPopTheNextFit() {
        ...
        ExecutorContext runningA1 = harness.start(flowA);
        ExecutorContext runningC = harness.start(flowC);
        Execution waitingA2 = Executions.created(flowA);
        harness.executionStateStore().save(waitingA2);
        assertThat(harness.handle(waitingA2)).executionInState(State.Type.QUEUED);
        Execution waitingB = Executions.created(flowB);
        harness.executionStateStore().save(waitingB);
        assertThat(harness.handle(waitingB)).executionInState(State.Type.QUEUED);
        ...
        Assertions.assertThat(popped).as("A2 — the oldest candidate — is still blocked by its own flow limit (A1 runs), so it is skipped and B pops instead of starving behind it")
            .map(Execution::getId).contains(waitingB.getId());
        assertThat(harness).hasQueuedExactly(waitingA2);
        ...
        Assertions.assertThat(reconsidered).as("the skipped A2 is reconsidered and finally pops").map(Execution::getId).contains(waitingA2.getId());
        assertThat(harness).hasNothingQueued();
        assertThat(harness).hasRunning(flowA, 1);
        assertThat(harness).hasRunning(namespaceScope, 2);
    }
    ...
    void shouldNotPopCandidateOutsideTheFreedScopes() {
        ...
        assertThat(harness.handle(waitingB2)).executionInState(State.Type.QUEUED);
        Execution waitingA2 = Executions.created(flowA);
        harness.executionStateStore().save(waitingA2);
        assertThat(harness.handle(waitingA2)).executionInState(State.Type.QUEUED);
        ...
        Assertions.assertThat(popped).as("A2 pops even though B2 queued earlier — B2 shares no scope with the freed slot").map(Execution::getId).contains(waitingA2.getId());
        assertThat(harness).hasQueuedExactly(waitingB2);
    }
    ...
    void shouldNotReleaseScopesWhenExecutionWasShortCircuited() {
        ...
        Execution second = Executions.created(flow);
        ExecutorContext cancelled = harness.handle(second);
        ...
        Assertions.assertThat(popped).as("the short-circuit guard holds — no decrement, no pop").as("nothing popped").isEmpty();
        assertThat(harness).hasRunning(namespaceScope, 1);
    }
