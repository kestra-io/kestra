    ...
    <data-table @onPageChanged="onPageChanged" ref="dataTable" :total="total">
      ...
      <template v-slot:table>
        ...
        >
          ...
          <template v-slot:cell(state.startDate)="row">{{row.item.state.startDate | date('LLLL')}}</template>
...
export default {
  ...
  methods: {
    onRowDoubleClick(item) {
        this.$router.push({ name: "executionEdit", params: {namespace: item.namespace, flowId: item.flowId, id: item.executionId},query: {tab:'gantt'}});
    },
    loadData(callback) {
      this.$store
