...
export function linkTag(md: MarkdownIt) {
    ...
    md.inline.ruler.after("emphasis", "link", (state, silent) => {
        ...
        const token = state.push("link_open", "link", 1);
        token.markup = "[[link]]";
        token.attrs = attrs;
        state.push("link_close", "link", -1);
    ...
    md.renderer.rules.link_close = () => "</router-md>";
}