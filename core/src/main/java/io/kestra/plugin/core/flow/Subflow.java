...
public class Subflow extends Task implements ExecutableTask<Subflow.Output>, ChildFlowInterface {
     ...
     */
    @Schema(title = "Outputs from the subflow executions.", description = "Allows to specify outputs as key-value pairs to extract any outputs from the subflow execution into output of this task execution. This property is deprecated since v0.15.0, please use the `outputs` property on the Subflow definition for defining the output values available and exposed to this task execution.")
    @PluginProperty(dynamic = true)
    ...
    @Schema(title = "What to do when a failed execution is restarting.", description = """
    ...
    public List<SubflowExecution<?>> createSubflowExecutions(
        ...
        return ExecutableUtils.subflowExecution(
            ...
            scheduleDate
        ).<List<SubflowExecution<?>>>map(List::of).orElse(Collections.emptyList());
    }
    ...
    public Optional<SubflowExecutionResult> createSubflowExecutionResult(
        ...
        boolean isOutputsAllowed = runContext.<Boolean>pluginConfiguration(PLUGIN_FLOW_OUTPUTS_ENABLED).orElse(true);

        final Output.OutputBuilder builder = Output.builder()
            .executionId(execution.getId())
            .state(execution.getState().getCurrent());

        final Map<String, Object> subflowOutputs = Optional.ofNullable(flow.getOutputs())
            .map(outputs -> outputs.stream().collect(Collectors.toMap(
