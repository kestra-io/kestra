<template>
    <div class="button-wrapper">
        ...
        <EditorButtons
            ...
            :isDraft="Boolean(flowStore.flow?.draft)"
            @save="save"
...
<script setup lang="ts">
    ...
    async function publish() {
        try {
            // Publish is only shown for an already-loaded, non-creating draft,
            // so save(false) here cannot return "redirect_to_update" — no redirect branch needed.
            const outcome = await flowStore.publish()
            if (isSuccessfulFlowSaveOutcome(outcome)) {
                onboardingStore.recordSave()
            }
            onSaveAll?.()
