...
class KoltpUtilsTest {
    ...
    void shouldCopyExecutableBinaryWhenResourceIsEmbedded() throws Exception {
        // When
        Path destination = KoltpUtils.copyTo(tempDir.resolve(KoltpUtils.BINARY_NAME));
    ...
    void shouldCreateParentDirectoriesWhenDestinationIsNested() throws Exception {
        // When
        Path destination = KoltpUtils.copyTo(tempDir.resolve("nested/dir/" + KoltpUtils.BINARY_NAME));
    ...
    void shouldOverwriteExistingFileWhenDestinationExists() throws Exception {
        // Given
        Path destination = tempDir.resolve(KoltpUtils.BINARY_NAME);
        Files.write(destination, new byte[]{1, 2, 3});

        // When
        KoltpUtils.copyTo(destination);
