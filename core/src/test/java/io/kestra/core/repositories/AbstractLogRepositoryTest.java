...
public abstract class AbstractLogRepositoryTest {
    ...
    void findAfterWithoutAcl_paginatesAcrossEqualTimestampsWithoutLossOrDuplicates() {
        // Given: five logs sharing the exact same timestamp (timestamp alone cannot paginate them), isolated by a
        // unique executionId. Distinct levels give each backend a working tiebreaker: JDBC seeks on the unique (timestamp, key) row.
        String executionId = "exec-keyset-" + IdUtils.create();
        ...
        for (Level level : List.of(Level.TRACE, Level.DEBUG, Level.INFO, Level.WARN, Level.ERROR)) {
            logRepository.save(logEntry(level, executionId).timestamp(ts).message("m-" + level.name()).build());
        }
        ...
        while (true) {
            List<LogRepositoryInterface.KeyedLog> page = logRepository.findAfterWithoutAcl(MAIN_TENANT, filters, afterTs, afterKey, 2);
            all.addAll(page);
            if (page.size() < 2) {
                break;
            }
