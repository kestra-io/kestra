...
public interface Namespace {
     ...
     * <p>
     * A path that holds no file resolves to its first version, so that callers can build the URI of a file
     ...
     * <p>
     * Deleting a file deletes every version it held at that point, so no version at or below the most
     * recent deletion of the path is ever served again — not even once the path has been re-created, which
     * only makes the versions written after that deletion readable.
     *
     * @param path the file path.
     ...
     *         held the given version or held it only before a deletion.
