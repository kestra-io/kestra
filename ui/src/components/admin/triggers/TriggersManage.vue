        ...
        triggerStore.setDisabled({...trigger, disabled: !value})
            .then((updatedTrigger: any) => {
                ...
                triggers.value = triggers.value?.map((tr: any) => {
                    const stateMatches = tr?.state &&
                        tr.state.namespace === updatedTrigger.namespace &&
                        tr.state.flowId === updatedTrigger.flowId &&
                        tr.state.triggerId === updatedTrigger.triggerId

                    if (stateMatches) {
                        return {...tr, state: updatedTrigger}
                    }
                    return tr
                })
            })
