...
public interface StatefulTriggerInterface {
    record Entry(String uri, String version, Instant modifiedAt, Instant lastSeenAt) {
    ...
    record StateUpdate(boolean fire, boolean isNew) {}

    default Map<String, Entry> readState(RunContext runContext, String key, Optional<Duration> ttl) {
        try {
    ...
    default void writeState(RunContext runContext, String key, Map<String, Entry> state, Optional<Duration> ttl) {
        try {
    ...
    default StateUpdate computeAndUpdateState(Map<String, Entry> state, Entry candidate, On on) {
    ...
    default boolean shouldFire(Entry prev, String version, On on) {
        if (prev == null) {
            return on == On.CREATE || on == On.CREATE_OR_UPDATE;
        }
        if (!Objects.equals(prev.version(), version)) {
            return on == On.UPDATE || on == On.CREATE_OR_UPDATE;
        }
        return false;
    }

    default String defaultKey(String ns, String flowId, String triggerId) {
