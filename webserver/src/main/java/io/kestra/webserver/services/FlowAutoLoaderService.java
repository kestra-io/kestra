                ...
                .flatMap(
                    ...
                    ).mapNotNull(response ->
                    {
                        ...
                        if (it.getId().equals(PURGE_SYSTEM_FLOW_BLUEPRINT_ID)) {
                            return NamespaceUtils.NAMESPACE_FROM_FLOW_SOURCE_PATTERN.matcher(Objects.requireNonNull(body))
                                .replaceFirst("namespace: " + namespaceUtils.getSystemFlowNamespace());
                        }
                        return body;
                    })
                )
