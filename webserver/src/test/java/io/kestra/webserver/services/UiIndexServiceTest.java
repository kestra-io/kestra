...
class UiIndexServiceTest {
    ...
    @Test
    void shouldGzipWhenAcceptedAndKeepContentLengthConsistent() throws Exception {
        // Given
        UiIndexService service = service(null, emptyConfiguration());

        // When
        MutableHttpResponse<byte[]> response = service
            .render(get("/ui/").header(HttpHeaders.ACCEPT_ENCODING, "gzip, deflate, br"))
            .orElseThrow();
        ...
        assertThat(new String(decompressed, StandardCharsets.UTF_8)).contains("window.KESTRA_UI_PATH = \"/ui/\";");
    }
