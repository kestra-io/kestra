...
<script lang="ts" setup>
    ...
    const forceRun = async () => {
      try {
        ...
        toast.error(t("force run failed"));
        console.error(err);
      }
    };
    ...
    const enabled = computed(() => {
      ...
      if (!user?.isAllowed(permission.EXECUTION, action.UPDATE, props.execution.namespace)) {
      return false;
      }

      return (
      State.isRunning(props.execution.state.current) || State.isQueued(props.execution.state.current)
      );
    });
