                    ...
                    <Column :label="t('settings.blocks.localization.fields.date_format')">
                        <KsSelect :modelValue="pendingSettings.dateFormat" @update:model-value="onDateFormat" :key="localeKey">
                            <KsOption
                                ...
                                :label="dateFilter(now.toISOString(), item.value)"
                                :value="item.value"
                            />
                        </KsSelect>
                    </Column>
...
<script setup lang="ts">
    ...
    import {executeFlowBehaviours, logDisplayTypes, storageKeys} from "../../utils/constants"
    ...
    const zonesWithOffset = moment.tz.names().map((zone: string) => {
    ...
    const canReadFlows = computed(() =>
        authStore.user?.isAllowed(resource.FLOW, action.VIEW, undefined),
    )
    ...
    // Initialise pendingSettings (was in created())
    ...
    pendingSettings.timezone = localStorage.getItem(storageKeys.TIMEZONE_STORAGE_KEY) || moment.tz.guess()
    ...
    pendingSettings.editorFontFamily = localStorage.getItem("editorFontFamily") || "'Source Code Pro', monospace"
    ...
    function isObjectEqual(obj1: Record<string, unknown>, obj2: Record<string, unknown>, keys: string[]) {
        ...
        if (hasUnsavedChanges.value) {
            ...
            if (shouldNavigate) {
                ...
                if (link.getAttribute("data-vue-router") === "true") {
                    // FIXME: router not available here without useRouter — rely on href navigation
    ...
    function onNamespaceSelect(value: string | string[] | undefined) {
        pendingSettings.defaultNamespace = typeof value === "string" ? value : undefined
        checkForChanges()
    }
    ...
    function onLevelChange(value: string | string[] | undefined) {
    ...
    function onTimezone(value: string | number) {
    ...
    function exportFlows() {
        return flowStore.findFlows({size: 1, page: 1})
            .then((result: { total: number }) => {
                const flowCount = result.total

                return flowStore.exportFlowByQuery({} as any)
                    .then(() => {
                        toast.success(
                            t("flows exported", {
                                count: flowCount,
        ...
        for (const key in pendingSettings) {
            ...
            case "defaultLogLevel":
                if (pendingSettings[key])
                    localStorage.setItem(key, pendingSettings[key] as string)
                else
                    localStorage.removeItem(key)
                break
            case "envName":
                if (pendingSettings[key] !== miscStore.configs?.environment?.name) {
                    layoutStore.setEnvName(pendingSettings[key] as string)
                }
                break
            case "envColor":
                if (pendingSettings[key] !== miscStore.configs?.environment?.color) {
                    layoutStore.setEnvColor(pendingSettings[key] as string)
                }
                break
            case "theme":
                Utils.switchTheme(miscStore, pendingSettings[key] as string)
            ...
            default:
                if (storedKey) {
                    if (pendingSettings[key])
                        localStorage.setItem(storedKey, pendingSettings[key] as string)
                } else {
