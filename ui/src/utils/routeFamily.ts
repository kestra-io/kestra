 ...
 * (see e.g. flowTabs.ts, executionTabs.ts, and kestra-ee's assetTabs.ts /
 * testSuiteTabs.ts). Once migrated, the active route's name becomes
 * `<family>/<tab>` instead of the flat `<family>` — this helper normalizes
 * back to the flat family name so route-identity checks written against the
