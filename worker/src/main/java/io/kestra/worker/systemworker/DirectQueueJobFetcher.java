 ...
 * keyed-dispatch queue with the reserved {@link WorkerGroup#SYSTEM_KEY}
 * routing key, bypassing the gRPC pull path used by regular workers.
 * <p>
...
public class DirectQueueJobFetcher extends WorkerLoop implements JobFetcher {
    ...
    public synchronized void init(final WorkerContext context) {
        ...
        this.subscription = workerJobQueue
            .subscriber(WorkerGroup.SYSTEM_KEY)
            .subscribe(this::onEvent);
        log.info("SystemWorker subscribed to routing key '{}'", WorkerGroup.SYSTEM_KEY);
    }
