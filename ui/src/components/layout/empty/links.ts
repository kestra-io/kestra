 ...
 *
 * Only set `video` when a recording covers that feature on its own; entries
 * without one show `learnMore` as a button instead.
 *
 * @todo Point every `learnMore` at a short.io link so the target can be changed
 * without a release.
 */
