...
// SDK. `createConfigureClient` builds an axios instance wired for the Kestra API (Content-Type /
// Accept fixes, the QueryFilter query serializer, Blob/string error normalization) and binds it to
// a hey-api-generated `client` singleton.
//
// It is generic over the client: @hey-api/client-axios vendors a self-contained `./openapi/client/`
// into each generated SDK, so there is no single "Client" type to import from here. TypeScript
// infers the exact per-SDK Client/Config type from whatever `client` is passed in at the call site.
//
// This intentionally holds NO module state: it just builds and returns the configured instance.
// The "current instance" singleton + accessors (useClient / setMockClient) are app concerns and
// live in the app (OSS/EE), not in this shared package.
