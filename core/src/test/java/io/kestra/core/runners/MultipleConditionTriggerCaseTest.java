...
public class MultipleConditionTriggerCaseTest {
    ...
    public void trigger() throws InterruptedException, TimeoutException {
        ...
        // first one
        Execution execution = runnerUtils.runOne("io.kestra.tests", "trigger-multiplecondition-flow-a", Duration.ofSeconds(60));
        ...
        // second one
        execution = runnerUtils.runOne("io.kestra.tests", "trigger-multiplecondition-flow-b", Duration.ofSeconds(60));
        assertThat(execution.getTaskRunList().size(), is(1));
        assertThat(execution.getState().getCurrent(), is(State.Type.SUCCESS));
