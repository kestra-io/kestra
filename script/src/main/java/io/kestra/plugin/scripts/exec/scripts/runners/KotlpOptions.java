 ...
 * @param forwardTraces whether to forward the instrumented spans (traces) received by kotlp to the Kestra tracing backend.
 *        If false, only logs and metrics will be processed.
 */
@Builder
public record KotlpOptions(Boolean enabled, String logDir, Duration logFlushInterval, Boolean forwardTraces) {

    public boolean isEnabled() {
        return Boolean.TRUE.equals(this.enabled);
    }

    public boolean isForwardTraces() {
        return Boolean.TRUE.equals(this.forwardTraces);
    }
}
