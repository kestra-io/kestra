    ...
    <section :class="{'container padding-bottom': topbar}" v-if="ready">
        ...
        >
            <template #navbar v-if="isDisplayedTop">
                <KSFilter
                    :configuration="namespace === undefined || flowId === undefined ? executionFilterConfiguration : flowExecutionFilterConfiguration"
                    :properties="{
                        shown: true,
...
<script setup lang="ts">
    ...
    import {executionFilterConfiguration} from "../filter/filter-v2/configurations/executionFilter";
    import {flowExecutionFilterConfiguration} from "../filter/filter-v2/configurations/flowExecutionfilter";
    import KSFilter from "../filter/filter-v2/components/KSFilter.vue";
    ...
    const props = defineProps({
        ...
        isConcurrency: {type: Boolean, default: false},
        id: {type: String, required:false, default: null},
        visibleCharts: {type: Boolean, default: false},
    });
    ...
    const optionalColumns = ref([
        {label: t("start date"), prop: "state.startDate", default: true},
        {label: t("end date"), prop: "state.endDate", default: true},
        {label: t("duration"), prop: "state.duration", default: true},
        {label: t("namespace"), prop: "namespace", default: true},
        {label: t("flow"), prop: "flowId", default: true},
        {label: t("labels"), prop: "labels", default: true},
        {label: t("state"), prop: "state.current", default: true},
        {label: t("revision"), prop: "flowRevision", default: false},
        {label: t("inputs"), prop: "inputs", default: false},
        {label: t("outputs"), prop: "outputs", default: false},
        {label: t("task id"), prop: "taskRunList.taskId", default: false}
    ]);
    ...
    const visibleColumnConfigs = computed(() => 
        displayColumns.value.map(prop => optionalColumns.value.find(c => c.prop === prop)).filter(Boolean) as any[]
    );
    ...
    const loadData = (callback?: () => void) => {
        ...
        executionsStore.findExecutions(loadQuery({
            size: parseInt(route.query.size as string || internalPageSize.value.toString()),
            page: parseInt(route.query.page as string || internalPageNumber.value.toString()),
    ...
    const {
        ready,
        internalPageSize: composablePageSize,
        internalPageNumber: composablePageNumber,
        onSort,
        onRowDoubleClick,
        onPageChanged,
        queryWithFilter,
    ...
    const internalPageSize = computed(() => composablePageSize.value);
    const internalPageNumber = computed(() => composablePageNumber.value);
    ...
    const filteredLabels = (labels: any[]) => {
        ...
        const allowedLabels = route.query.labels ? (Array.isArray(route.query.labels) ? route.query.labels : [route.query.labels]).filter((label): label is string => label !== null).map((label: string) => label.split(":")[0]) : [];
    ...
    const refresh = () => {
        ...
        const dashboardComponent = selectTable.value?.$refs?.dashboardComponent;
    ...
    onMounted(() => {
        ...
        if (route.name === "flows/update") {
            optionalColumns.value = optionalColumns.value.filter(col => col.prop !== "namespace" && col.prop !== "flowId");
        }
    });
