...
public record ApiMcpServer(
    ...
    @NotNull @NotBlank @Pattern(regexp = "^[a-z0-9][a-z0-9_-]*") String id,

    @Schema(description = "Human-readable description of the MCP server.") String description,

    @Schema(description = "Instructions sent to the AI model when using this server.") String instructions,

    @Schema(description = "Visibility of the server.") McpServer.ServerType serverType,

    @Schema(description = "Authentication type for private servers.") McpServer.AuthType authType,

    @Schema(description = "OAuth provider key from micronaut.security.oauth2.clients. Required when authType is OAUTH.") String oauthProvider,
    ...
    @Schema(description = "Timestamp when the server was created.", accessMode = Schema.AccessMode.READ_ONLY) Instant created,

    @Schema(description = "Timestamp when the server was last updated.", accessMode = Schema.AccessMode.READ_ONLY) Instant updated) {
    /**
