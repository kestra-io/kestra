...
async function requestTranslation(client: TranslationClient, text: string, targetLanguage: string): Promise<string | undefined> {
    const prompt = `Translate the text provided after "----------" into ${targetLanguage} for use in Kestra’s orchestration UI. Follow these guidelines:
        ...
        - Polish Declension of Reserved Terms: Polish inflects nouns by case, so the reserved English terms have to be declined, not left in the nominative. Add the ending directly where the stem allows it ("Taska", "Taskiem", "Tasków", "Executiona", "Executionów", "Triggera", "Workerów", "Secretów", "Assetów", "Labeli"), and use an apostrophe where the stem does not fit Polish inflection ("namespace'u", "namespace'ie", "namespace'ów", "Case'a", "Case'ów"). "Flow" stays uninflected in the singular ("tego Flow", "w tym Flow") and takes a Polish plural ("Flowy", "Flowów"). All of these are masculine.
        - Polish Glossary: Execution (never "wykonanie" for the entity, and never "egzekucja", which means an execution by firing squad), Task (never "zadanie"), Flow (never "przepływ"), Trigger (never "wyzwalacz"), Namespace (never "przestrzeń nazw"), Secret (never "sekret" or "tajemnica"), Case (never "przypadek", "sprawa" or "zgłoszenie"), Asset (never "zasób" when it means the Kestra entity), Credential (never "poświadczenie"), Policy (never "polityka" or "zasada"), Quota (never "kwota", which means a sum of money), Storage (never "pamięć masowa" or "magazynowanie"), Playground (never "plac zabaw"), Payload (never "ładunek"), Provisioning (never "świadczenie"), Dashboard (never "pulpit nawigacyjny"), Heartbeat (never "bicie serca"), Board, Fixtures, Timeline. "Kill" is "zatrzymać", never "zabić". "Impersonate" is "przejąć tożsamość", never "podszyć się". "Resolution" as an outcome is "rozwiązanie", never "rozdzielczość". Use "współbieżność" for concurrency and "zakładka" for tab, both of which are natural Polish. Keep "Cron" as "Cron", never "Kron".
          ...
          - Polish keeps "kill" and "stop" apart: "zabić" / "Zabij" for the kill action, "zatrzymać" for stop. Collapsing them produces nonsense like "musisz zatrzymać Execution, aby go zatrzymać".
        - Polish Prefers a Participle to a Short Relative Clause: when the English is a short noun phrase with a passive relative clause, use the Polish participle. "Executions triggered from Playground mode" is "Executiony uruchomione w trybie Playground", not "Executiony, które zostały uruchomione w trybie Playground". Keep the relative clause when the modifier is long.
        ...
        - State Labels in English: Keep status labels that are in all caps (e.g. WARNING, FAILED, SUCCESS, PAUSED, RUNNING) in English and in their original uppercase format. This applies to badges and status values only — never rewrite running prose into an all-caps state, so "Execution is paused" is "Execution został wstrzymany", not "Execution jest PAUSED".
