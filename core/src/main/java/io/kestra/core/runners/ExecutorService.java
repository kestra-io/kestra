        ...
        for (TaskRun taskRun : executor.getExecution().getTaskRunList()) {
            ...
            if (!executor.getExecution().getState().isRetrying() &&
                ...
                if (executor.getFlow().findTaskByTaskId(taskRun.getTaskId()).getRetry() != null) {
                    ...
                    nextRetryDate = taskRun.nextRetryDate(executor.getFlow().findTaskByTaskId(taskRun.getTaskId()),
                        behavior.equals(AbstractRetry.Behavior.NEW_EXECUTION) ? executor.getExecution().getAttemptNumber() : null);
                ...
                // Case flow has a retry
                    ...
                    nextRetryDate = behavior.equals(AbstractRetry.Behavior.NEW_EXECUTION) ?
                        executionService.nextRetryDate(executor.getFlow(), executor.getExecution()) :
