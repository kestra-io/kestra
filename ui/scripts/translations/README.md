...
This branch carries the same scripts, the same rules and the same workflows as `develop` and `releases/v2.0.x`, with these differences:

- **No design system.** The branch consumes `@kestra-io/ui-libs` from npm and has no `ui/packages/`, so phase 2 of the generator (the `*.locale.ts` files) and `fingerprints-design-system.json` have nothing to act on; the design-system rows in the tables above do not apply. The workflows stage `ui/src/translations/*.json` and `fingerprints.json` only.
- **The unused-key rule is not wired.** It needs the EE tree in view and an `en.json` cleaned up the way `develop` was ([kestra#19510](https://github.com/kestra-io/kestra/pull/19510)); on this branch the OSS jobs do not check EE out, and the EE job skips the rule because the OSS source roots it expects (`ui/packages/*`) do not exist here. Every other rule - parity, placeholders, English copies, staleness, used-but-undefined keys and namespaces, EE keys shadowing OSS ones - is enforced.
- **Node.** `ui/.nvmrc` still says 22.13, where importing the `.ts` rules from the `.mjs` gate needs `--experimental-strip-types`; the npm scripts pass it (a no-op on 24), and the translation workflows run on Node 24 like the branch's frontend workflows already do. `package.json` declares no `engines` field here.
- **No runtime guards in the test suites.** The app reports a key it cannot resolve on the console (and throws under Vitest) exactly as on `develop`, but the branch has no shared e2e `auth.ts` fixture to fail a test on it, and the i18n convention guard (`<i18n-t>`, `t()` in templates) is not backported: the branch still uses both.
- **The scheduled bot** runs from `develop`'s copy of `auto-translate-ui-keys.yml`; the copy here lists all three branches so a manual run from this branch behaves the same.
