...
-- to match Task.id's / Trigger.id's @Size(max = 256). Guarded: modifying a STORED GENERATED column
-- rebuilds the table (ALGORITHM=COPY), so each widens only when still shorter than 256. ${table} is
-- substituted with the configured log table name.
