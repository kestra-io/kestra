...
- **PR comment.** `translations-pr-comment.yml` runs after the PR workflow (`workflow_run`, so it has a token that can write even for a fork) and posts one sticky comment with the gate's findings from the report the `translations` job uploads, deleted again once the check passes. On a pull request from a fork it also explains how to get the missing translations generated: the on-demand workflow for a maintainer, or "Allow edits from maintainers", or a local run with a key.
