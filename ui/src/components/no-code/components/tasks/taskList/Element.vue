<template>
    <div @click="handleClick" class="d-flex my-2 p-2 rounded element" :class="{'moved': moved}">
        <div v-if="!['inputs', 'layout'].includes(props.parentPathComplete)" class="me-2 icon">
            <KsTaskIcon v-if="element.type" :cls="element.type" :icons="pluginsStore.icons" onlyIcon />
            <PlusBoxOutline v-else class="placeholder-icon" />
        </div>

        <div class="flex-grow-1 label" :class="{placeholder: !element.type}">
            {{ element.type ? (title ?? identifier) : placeholderLabel }}
        </div>
...
<script setup lang="ts">
    ...
    const placeholderLabel = computed(() => {
        ...
        return /^\d*$/.test(field) ? (props.title ?? identifier.value) : `${t("add")} ${capitalCase(field)}`
    })
