...
public class CurrentOutputFunction implements Function {

    @Override
    public Object execute(Map<String, Object> args, PebbleTemplate self, EvaluationContext context, int lineNumber) {
        if (!args.containsKey("outputs")) {
            throw new PebbleException(null, "The 'currentOutput' function expects an argument 'outputs'.", lineNumber, self.getName());
        }

        if (!(args.get("outputs") instanceof Map)) {
            throw new PebbleException(null, "The 'currentOutput' function expects an argument 'outputs' with type map.", lineNumber, self.getName());
        }
