...
watch(() => props.dirty, (newVal) => {
    if (!newVal && !props.flow) savedSourceNS.value = sourceNS.value;
});

onMounted(() => {
    if (props.flow) pluginsStore.lazyLoadSchemaType({ type: "flow" });
    loadFile();
    loadPluginsHash();
...
// --- AI Copilot unmount on route change ---
onBeforeRouteLeave(() => {
    aiCopilotOpened.value = false;
});

// --- Language Map ---
const LANGS_WITH_WORKERS_MAP = { yaml: "yaml", yml: "yaml", json: "json", js: "javascript", ts: "typescript", jsx: "javascript", tsx: "typescript" };
const lang = computed(() => LANGS_WITH_WORKERS_MAP[props.extension as keyof typeof LANGS_WITH_WORKERS_MAP] || undefined);

onActivated(loadFile);
onBeforeUnmount(() => {
    aiCopilotOpened.value = false;
...
const editorContent = computed(() => draftSource.value ?? source.value);
const updateContent = inject(FILES_UPDATE_CONTENT_INJECTION_KEY);

function editorUpdate(newValue: string) {
    if (editorContent.value === newValue) return;
    if (props.flow) {
        if (hasDraft.value) draftSource.value = newValue;
        else flowStore.flowYaml = newValue;
    }
    sourceNS.value = newValue;
    if (props.path) updateContent?.({ path: props.path, content: newValue });

    if (!props.flow) return;
    clearTimeout(timeout.value);
    timeout.value = setTimeout(() => {
        flowStore.onEdit({ source: newValue, editorViewType: "YAML", topologyVisible: true });
    }, 1000);
}
...
function updatePluginDocumentation(event: any) {
    ...
    const isInRange = (range: [number, number, number]) => cursorOffset >= range[0] && cursorOffset <= range[2];
...
const saveFlowYaml = async () => {
    ...
    const result = flowStore.isCreating ? await flowStore.save() : await flowStore.saveAll();
    if (result === "redirect_to_update") {
        await router.push({ name: "flows/update", params: { id: flowStore.flow?.id, namespace: flowStore.flow?.namespace, tab: "edit", tenant: route.params?.tenant } });
    }
};
...
const saveFileContent = async () => {
    ...
    await namespacesStore.createFile({ namespace: namespace.value, path: props.path, content: editorContent.value || "" });
    savedSourceNS.value = source.value;
};
...
const handleGlobalSave = (event: KeyboardEvent) => {
    if ((event.ctrlKey || event.metaKey) && event.key === "s") {
        ...
        props.flow ? saveFlowYaml() : saveFileContent();
    }
};
...
const execute = () => (flowStore.executeFlow = true);
...
function closeAiCopilot() {
    ...
    router.replace({ name: route.name, params: route.params, query: currentQuery });
}
const hasDraft = computed(() => draftSource.value !== undefined);
const { playgroundStore, highlightHoveredTask, highlightedLines } = useFlowEditorRunTaskButton(computed(() => props.flow), editorRefElement, source);
</script>
