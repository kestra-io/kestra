...
public class TaskOutputService extends AbstractOutputService {
    ...
    /**
     * Delete outputs for a given list of task runs.
     * This will also delete any internal storage files associated with these outputs or generated by the task runs.
     */
    public void deleteByTaskRun(Execution execution, List<TaskRun> taskRuns) {
        ...
        for (TaskRun taskRun : taskRuns) {
            try {
                java.net.URI taskStorageUri = java.net.URI.create(
                    StorageContext.KESTRA_PROTOCOL + StorageContext.forTask(taskRun).getContextStorageURI().getPath()
                );
                storageInterface.deleteByPrefix(execution.getTenantId(), execution.getNamespace(), taskStorageUri);
            } catch (Exception e) {
