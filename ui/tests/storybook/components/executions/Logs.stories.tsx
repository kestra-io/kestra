    ...
    return [
        () => ({
            setup() {
                ...
                executionsStore.flow = {tasks: execution.taskRunList.map(task => ({id: task.taskId, type: "io.kestra.plugin.core.log.Log"}))} as any;
...
function compactFullscreenPlay(lastTaskId = "task-3") {
    return async ({canvasElement}: {canvasElement: HTMLElement}) => {
        ...
        compactScroller.scrollTop = 1000;
        compactScroller.dispatchEvent(new Event("scroll"));
        await waitFor(() => expect(compactScroller.scrollTop).toBeGreaterThan(900));
