...
class ErrorsAndFinallyTest {
    ...
    void shouldRunErrorsBranchThenFailWhenMainTaskFails() throws Exception {
        ...
        assertThat(cycle3).as("the execution still ends FAILED — a successful errors branch does not rescue it")
    ...
    void shouldNotRunErrorsBranchWhenMainTaskSucceeds() throws Exception {
        ...
        assertThat(cycle2).as("straight to SUCCESS — the error task is never emitted and no taskrun exists for it")
    ...
    void shouldRunFinallyTaskThenSucceedWhenMainTaskSucceeds() throws Exception {
        ...
        assertThat(cycle3).as("everything green — SUCCESS")
    ...
    void shouldRunFinallyTaskThenFailWhenMainTaskFails() throws Exception {
        ...
        assertThat(cycle3).as("the main-task failure still decides the final state — FAILED")
            .hasTaskRunInState(FINALLY_TASK, State.Type.SUCCESS)
            .executionInState(State.Type.FAILED)
            .transitioned(State.Type.RUNNING, State.Type.FAILED);
    }
    ...
    void shouldEmitAfterExecutionTaskOnlyAfterExecutionReachesTerminalState() throws Exception {
        ...
        assertThat(cycle2).as(
            "the state transition ordering proves terminal-before-afterExecution — the cycle transitioned RUNNING → SUCCESS and the `from` audit trail shows onEnd contributed before handleAfterExecution did"
        )
            .executionInState(State.Type.SUCCESS)
            .transitioned(State.Type.RUNNING, State.Type.SUCCESS)
            .updatedFrom("onEnd")
        ...
        assertThat(cycle3).as("the execution state is untouched by the afterExecution outcome")
