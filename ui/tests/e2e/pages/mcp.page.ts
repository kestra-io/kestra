...
export class McpPage extends BasePage {
    ...
    // Navigate to the create page via the "Create Server" button in the nav bar
    ...
    // Find a server row on the list page by server id
    ...
    // Navigate to a server's detail page (lands on the Edit tab)
    ...
    // Navigate to a server's detail page then click the Connect tab
    async openConnectModal(id: string): Promise<void> {
        await this.openEditModal(id);
        await this.page.getByRole("tab", {name: "Connect"}).click();
    }

    // Navigate to the edit tab, toggle the enabled switch, save, then return to the list
    ...
    // Navigate to the edit tab and click Delete (accepts the confirm dialog)
    ...
    // Check whether the server row on the list shows an "Enabled" status badge
