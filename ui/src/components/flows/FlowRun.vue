<template>
    <template v-if="flow">
        ...
        <div class="flow-execution-checks-alerts">
            <el-alert
                v-for="alert in checks || []"
                :type="alert.style.toLowerCase()"
                showIcon
                :closable="false"
                :key="alert"
            >
        ...
        >
            <InputsForm
                ...
                @update:model-value-no-default="
                    (values) => (inputsNoDefaults = values)
                "
            ...
            <el-collapse v-model="collapseName">
                ...
                >
                    ...
                    <el-form-item :label="$t('scheduleDate')">
                        <el-date-picker
                ...
                <el-collapse-item
                    v-if="hasWebhookTriggers"
                    :title="$t('webhook.curl_command')"
                    name="webhook-curl"
                >
                ...
                <div class="left-align">
                    <el-form-item>
                        <el-button
                            v-if="
                                execution &&
                                    (execution.inputs || hasExecutionLabels())
                            "
                            :icon="ContentCopy"
                            @click="fillInputsFromExecution"
                        >
                            {{ $t("prefill inputs") }}
                        </el-button>
                    </el-form-item>
                ...
                <div class="right-align">
                    <el-form-item class="submit">
                        <el-button
                            :icon="buttonIcon"
                            :disabled="
                                !flowCanBeExecuted || hasBlockingChecks()
                            "
                            :class="{
                                'flow-run-trigger-button': true,
                                'onboarding-glow':
    ...
    export default {
        ...
        data() {
            return {
                ...
                newTab:
                    localStorage.getItem(storageKeys.EXECUTE_FLOW_BEHAVIOUR) ===
                    executeFlowBehaviours.NEW_TAB,
                executeClicked: false,
                checks: [],
            };
        },
        emits: ["execution-trigger", "updateInputs", "updateLabels"],
        computed: {
            ...mapStores(
                useCoreStore,
                useMiscStore,
            ...
            haveBadLabels() {
                return this.executionLabels.some(
                    (label) =>
            ...
            hasWebhookTriggers() {
                ...
                return this.flow.triggers.some(
                    (trigger) =>
        ...
        methods: {
            hasBlockingChecks() {
                return (
                    this.checks.filter(
                        (check) => check.behavior === "BLOCK_EXECUTION",
                    ).length > 0
                );
            },
            getExecutionLabels() {
                ...
                return this.execution.labels.filter((label) => {
                    return !this.flow.labels.some(
                        (flowLabel) =>
                            flowLabel.key === label.key &&
                            flowLabel.value === label.value,
                    );
                });
            },
            hasExecutionLabels() {
            ...
            fillInputsFromExecution() {
                ...
                this.executionLabels = this.getExecutionLabels().filter(
                    (item) =>
                        !toIgnore.some((prefix) => item.key.startsWith(prefix)),
                );
                if (!this.flow?.inputs) return;
                this.flow.inputs.forEach((input) => {
                    const value =
                        this.execution.inputs?.[input.id] ?? input.default ?? "";
                    this.inputs[input.id] = Inputs.normalize(input.type, value);
                });
            },
            onSubmit(formRef) {
                if (!formRef) {
                    console.warn("FlowRun: form ref not ready yet");
                    return;
                }
                if (this.flowCanBeExecuted) {
                    this.checks = [];
                    ...
                    formRef.validate((valid) => {
                        ...
                        if (this.replaySubmit) {
                            this.replaySubmit({
                                ...
                                inputs: this.selectedTrigger?.inputs
                                    ? {
                                        ...this.selectedTrigger.inputs,
                                        ...this.inputsNoDefaults,
                                    }
                                    : this.inputsNoDefaults,
                                labels: [
                                    ...new Set(
                            ...
                            executeTask(
                                    ...
                                    : this.inputsNoDefaults,
                                {
                                    ...
                                    scheduleDate: this.$moment(this.scheduleDate)
                                        .tz(
                                            localStorage.getItem(
                                                storageKeys.TIMEZONE_STORAGE_KEY,
                                            ) ?? moment.tz.guess(),
                                        )
                                        .toISOString(true),
                                    nextStep: true,
