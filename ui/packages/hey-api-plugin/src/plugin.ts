...
export const handler: KestraSdkPlugin["Handler"] = ({plugin}) => {
    // Stamp the OpenAPI spec hash into the generated SDK (no external bin / postProcess needed).
    // sha256 of the RAW spec file — matching what the backend serves — so the dev-time staleness
    // check can fetch the live spec and compare. Emitting it as an exported symbol in sdk/shared
    // means the generated index re-exports it automatically (like the tenant helpers below).
    const specPath = plugin.config.specPath
    if (specPath) {
        const hash = createHash("sha256").update(readFileSync(specPath)).digest("hex").slice(0, 16)
        const hashSymbol = plugin.symbol("OPENAPI_SPEC_HASH", {
            getFilePath: () => "sdk/shared",
        })
        plugin.node($.const(hashSymbol).export().assign($.literal(hash)))
    }
