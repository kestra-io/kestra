...
public class Switch extends Task implements FlowableTask<Switch.Output> {
    ...
    public List<ResolvedTask> childTasks(RunContext runContext, TaskRun parentTaskRun) throws IllegalVariableEvaluationException {
        for (Map.Entry<String, List<Task>> entry : cases.entrySet()) {
            if (entry.getKey().equals(rendererValue(runContext))) {
                List<Task> tasks = entry.getValue();
                return FlowableUtils.resolveTasks(tasks, parentTaskRun);
            }
        }

        return FlowableUtils.resolveTasks(this.defaults, parentTaskRun);
    }
