...
<script setup lang="ts">
    import {computed, onMounted, Ref, watch, ref, provide} from "vue";
    ...
    function findTopologyIndexes(arr: { tabs: { value: string }[] }[]): { panelIndex: number, tabIndex: number } {
        const panelIndex = arr.findIndex(p => p.tabs.some(t => t.value === "topology"));
        const tabIndex = panelIndex !== -1 ? arr[panelIndex].tabs.findIndex(t => t.value === "topology") : 0;
    ...
    watch(topologyClick, (value: TopologyClickParams | undefined) => {
        ...
        if(!visible.includes("code") && !visible.includes("nocode")){
            const {action, params: {section, id}} = value;
