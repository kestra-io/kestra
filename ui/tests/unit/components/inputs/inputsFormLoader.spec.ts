...
describe("InputsForm computing-values state", () => {
    ...
    test("keeps a loading spinner on dynamic inputs during recomputations, without disabling them", async () => {
        // Given: an initial fetch then a (pending) recomputation
        const store = useExecutionsStore()
        ...
        // After the initial fetch completes (response carries the rendered inputs)
        ...
        // Then: the spinner shows (values may change) but the input is no longer disabled
        expect(wrapper.vm.isLoadingInput("datacenter")).toBe(true)
        expect(wrapper.vm.isComputingInput("datacenter")).toBe(false)
    })
