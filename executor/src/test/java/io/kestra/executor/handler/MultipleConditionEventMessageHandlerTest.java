...
class MultipleConditionEventMessageHandlerTest {
    ...
    @Test
    void shouldCarryExecutionDepthAcrossToTheCreateCommand() {
        ...
        // returned by FlowTriggerService into a Create command
        var flow = Fixtures.flow();
        harness.registerFlow(Flows.of(flow));
        var upstream = Execution.newExecution(flow, List.of());
        harness.executionStateStore().save(upstream);
        var triggered = Execution.newExecution(flow, List.of());
        triggered = triggered.withMetadata(triggered.getMetadata().withExecutionDepth(4));

        when(harness.flowTriggerService().computeExecutionsFromFlowTriggerDependsOn(upstream, flow, harness.multipleConditionStateStore()))
            .thenReturn(List.of(triggered));

        // When
        harness.multipleConditionEventMessageHandler().handle(new MultipleConditionEvent(flow, upstream));

        // Then the Create command carries the depth onward, rather than resetting it to a fresh root
        assertThat(harness.executionCommandQueue().emitted()).hasSize(1);
        var create = (Create) harness.executionCommandQueue().emitted().getFirst();
        assertThat(create.executionDepth()).isEqualTo(4);
    }
    ...
    void shouldEmitNothingWhenNoExecutionIsComputed() {
        ...
        // When
        harness.multipleConditionEventMessageHandler().handle(new MultipleConditionEvent(flow, execution));
        assertThat(harness).emittedNoCommands();
    }
}
