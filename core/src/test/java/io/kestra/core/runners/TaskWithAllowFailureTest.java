...
public class TaskWithAllowFailureTest {
    ...
    @Test
    @LoadFlows(
        { "flows/valids/task-allow-failure-executable-flow.yml",
    ...
    @Test
    @LoadFlows(
        { "flows/valids/task-allow-failure-executable-foreachitem.yml",
            "flows/valids/for-each-item-subflow-failed.yaml" }
    )
    void executableTask_ForEachItem() throws TimeoutException, QueueException, URISyntaxException, IOException {
        ...
        Execution execution = runnerUtils
            .runOne(MAIN_TENANT, "io.kestra.tests", "task-allow-failure-executable-foreachitem", null, (flow, execution1) -> flowIO.readExecutionInputs(flow, execution1, inputs));
